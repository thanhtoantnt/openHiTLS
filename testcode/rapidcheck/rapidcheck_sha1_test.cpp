/**
 * @file rapidcheck_sha1_test.cpp
 * @brief RapidCheck property-based tests for SHA-1 hash function
 * 
...
int main() {
    /**
     * @test SHA-1 hash is deterministic
     * @property For all inputs, hash(input) == hash(input)
     * @generalizes SDV_CRYPT_EAL_SHA1_API_TC002 - Update and final test
     * @see testcode/sdv/testcase/crypto/sha1/test_suite_sdv_eal_md_sha1.c:84-120
     */
    rc::check("SHA-1 hash is deterministic",
        [](const std::vector<uint8_t> &input) {
            CRYPT_EAL_MdCtx *ctx1 = CRYPT_EAL_MdNewCtx(CRYPT_MD_SHA1);
            CRYPT_EAL_MdCtx *ctx2 = CRYPT_EAL_MdNewCtx(CRYPT_MD_SHA1);
            RC_PRE(ctx1 != nullptr);
            RC_PRE(ctx2 != nullptr);
            ...
            RC_ASSERT(len1 == 20);
            RC_ASSERT(len2 == 20);
            RC_ASSERT(std::memcmp(hash1, hash2, 20) == 0);
    ...
    /**
     * @test SHA-1 output size is always 20 bytes
     * @property For all inputs, len(hash(input)) == 20
     * @generalizes SDV_CRYPT_EAL_SHA1_API_TC002 - Digest length test
     * @see testcode/sdv/testcase/crypto/sha1/test_suite_sdv_eal_md_sha1.c
     */
    rc::check("SHA-1 output size is always 20 bytes",
        [](const std::vector<uint8_t> &input) {
            RC_ASSERT(CRYPT_EAL_MdGetDigestSize(CRYPT_MD_SHA1) == 20);
    ...
    /**
     * @test SHA-1 incremental hashing equals single update
     * @property hash(a+b) == hash(a) then hash(b)
     * @generalizes SDV_CRYPT_EAL_SHA1_FUNC_TC003 - Multi-update test
     * @see testcode/sdv/testcase/crypto/sha1/test_suite_sdv_eal_md_sha1.c
     */
    rc::check("SHA-1 incremental hashing equals single update",
        [](const std::vector<uint8_t> &input) {
            RC_PRE(input.size() > 1);
    ...
    /**
     * @test SHA-1 different inputs produce different hashes
     * @property For distinct input pairs, hash(a) != hash(b)
     * @generalizes Collision resistance test
     * @see testcode/sdv/testcase/crypto/sha1/test_suite_sdv_eal_md_sha1.c
     */
    rc::check("SHA-1 different inputs produce different hashes",
    ...
    /**
     * @test SHA-1 copy context produces same hash
     * @property For all inputs, hash_dup(ctx) == hash(ctx)
     * @generalizes SDV_CRYPT_EAL_SHA1_COPY_CTX_FUNC_TC001 - Copy context test
     * @see testcode/sdv/testcase/crypto/sha1/test_suite_sdv_eal_md_sha1.c
     */
    rc::check("SHA-1 copy context produces same hash",
        [](const std::vector<uint8_t> &input) {
            CRYPT_EAL_MdCtx *ctx1 = CRYPT_EAL_MdNewCtx(CRYPT_MD_SHA1);
            RC_PRE(ctx1 != nullptr);
